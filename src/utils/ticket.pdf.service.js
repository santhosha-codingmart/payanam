// =============================================================================
// Ticket PDF Generator — plain, readable PDF ticket using PDFKit
//
// DESIGN: Black text on white background. No colors, no emoji.
//         Standard Helvetica fonts only (safe for all PDF viewers).
//
// RETURNS: Promise<Buffer> — raw PDF bytes for:
...
/**
 * Safely extract a city string from a route endpoint.
 * The schema stores source/destination as { city, state } objects.
 */
const cityOf = (val) => {
    if (!val) return "N/A";
...
export const generateTicketPDF = (booking) =>
    new Promise((resolve, reject) => {
        try {
            ...
            // user.name comes from .populate("userId", "name email").
            // If not populated or user deleted, fall back to the first passenger's name.
            ...
            // source and destination are { city, state } objects
            ...
            const W   = 595.28 - 100; // usable width (A4 minus margins)
            const L   = 50;           // left edge (margin)
            ...
            // ── Helper: labelled field (left-aligned) ─────────────────────────
            const field = (label, value, x, y, maxW) => {
                doc.font("Helvetica").fontSize(7).fillColor("#000000")
                   .text(label.toUpperCase(), x, y);
                doc.font("Helvetica-Bold").fontSize(11).fillColor("#000000")
                   .text(value, x, y + 11, { width: maxW, lineBreak: false });
            };

            // ── HEADER ────────────────────────────────────────────────────────
            doc.font("Helvetica-Bold").fontSize(22).fillColor("#000000")
               .text("PAYANAM", L, 40, { align: "center", width: W });

            doc.font("Helvetica").fontSize(9).fillColor("#000000")
               .text("Bus Ticket / Boarding Pass", L, 66, { align: "center", width: W });
            ...
            doc.font("Helvetica-Bold").fontSize(10)
               .text(`Booking Ref: ${bookingId}`, L, y, { align: "right", width: W });
            ...
            // From | Departure Date | Departure Time | Arrival Time | To
            const col = W / 5;
            ...
            // Table header
            const th = { name: L, seat: L + 200, age: L + 280, gender: L + 340 };
            ...
            passengerDetails.forEach((p, i) => {
                ...
                doc.fillColor("#000000")
                   .text(p.name || "N/A", th.name,   y, { width: 185, lineBreak: false })
                   .text(seat,             th.seat,   y)
            ...
            y += 20;
            doc.font("Helvetica-Bold").fontSize(9).fillColor("#000000")
               .text("Safe Travels with Payanam!", L, y, { width: W, align: "center" });
